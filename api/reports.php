...
function getAllBranchesRevenueReport() {
    ...
    $monthly_query = "
        SELECT 
            ...
            CAST(SUM(i.amount) AS DECIMAL) AS total
        FROM invoices i
        $where_clause
    ...
    try {
        // Tổng doanh thu
        $total_stmt = $pdo->prepare("SELECT CAST(SUM(i.amount) AS DECIMAL) AS total_revenue FROM invoices i $where_clause");
...
function getContractReport($branchId) {
    ...
    $conditions = ['c.branch_id = ? AND c.deleted_at IS NULL'];
    $params = [$branchId];
...
function getAssignedBranchesRevenueReport($employeeId) {
    ...
    $conditions = ['i.branch_id IN (SELECT branch_id FROM branch_employees WHERE employee_id = ?) AND i.deleted_at IS NULL'];
    $params = [$employeeId];
...
function getAssignedBranchesRoomStatusReport($employeeId) {
    ...
    $conditions = ['r.branch_id IN (SELECT branch_id FROM branch_employees WHERE employee_id = ?) AND r.deleted_at IS NULL'];
    $params = [$employeeId];
    ...
    try {
        ...
        $stats_stmt = $pdo->prepare("
            SELECT 
                ...
                SUM(CASE WHEN r.status = 'maintenance' THEN 1 ELSE 0 END) AS maintenance_rooms
            FROM rooms r
            WHERE r.branch_id IN (SELECT branch_id FROM branch_employees WHERE employee_id = ?) AND r.deleted_at IS NULL
        ");
...
function getAssignedBranchesContractReport($employeeId) {
    ...
    $conditions = ['c.branch_id IN (SELECT branch_id FROM branch_employees WHERE employee_id = ?) AND c.deleted_at IS NULL'];
    $params = [$employeeId];
    ...
    try {
        ...
        $stats_stmt = $pdo->prepare("
            SELECT 
                ...
                SUM(CASE WHEN c.status = 'cancelled' THEN 1 ELSE 0 END) AS cancelled_contracts
            FROM contracts c
            WHERE c.branch_id IN (SELECT branch_id FROM branch_employees WHERE employee_id = ?) AND c.deleted_at IS NULL
        ");
        $stats_stmt->execute([$employeeId]);
        $stats = $stats_stmt->fetch(PDO::FETCH_ASSOC);
...
function getAssignedBranchesUtilityUsageReport($employeeId) {
    ...
    $conditions = ['u.room_id IN (SELECT id FROM rooms WHERE branch_id IN (SELECT branch_id FROM branch_employees WHERE employee_id = ?) AND deleted_at IS NULL) AND u.deleted_at IS NULL'];
    $params = [$employeeId];
    ...
    try {
        ...
        $stats_stmt = $pdo->prepare("
            ...
            JOIN services s ON u.service_id = s.id
            JOIN rooms r ON u.room_id = r.id
            WHERE r.branch_id IN (SELECT branch_id FROM branch_employees WHERE employee_id = ?) AND u.deleted_at IS NULL
            GROUP BY s.type
        ");
        $stats_stmt->execute([$employeeId]);
        $stats = $stats_stmt->fetchAll(PDO::FETCH_ASSOC);
...
function getAssignedBranchesMaintenanceReport($employeeId) {
    ...
    $conditions = ['mr.room_id IN (SELECT id FROM rooms WHERE branch_id IN (SELECT branch_id FROM branch_employees WHERE employee_id = ?) AND deleted_at IS NULL) AND mr.deleted_at IS NULL'];
    $params = [$employeeId];
    ...
    try {
        ...
        $stats_stmt = $pdo->prepare("
            SELECT 
                ...
                SUM(CASE WHEN mr.status = 'completed' THEN 1 ELSE 0 END) AS completed_requests
            FROM maintenance_requests mr
            WHERE mr.room_id IN (SELECT id FROM rooms WHERE branch_id IN (SELECT branch_id FROM branch_employees WHERE employee_id = ?) AND deleted_at IS NULL) AND mr.deleted_at IS NULL
        ");
        $stats_stmt->execute([$employeeId]);
        $stats = $stats_stmt->fetch(PDO::FETCH_ASSOC);
...
function getCustomerContracts($userId) {
    ...
    $query = "
        SELECT 
            c.id, r.name AS room_name, c.start_date, c.end_date, c.status, c.deposit
        FROM contracts c
        JOIN rooms r ON c.room_id = r.id
        $where_clause
        ORDER BY c.created_at DESC
...
function getCustomerInvoices($userId) {
    ...
    $query = "
        SELECT 
            i.id, r.name AS room_name, CAST(i.amount AS DECIMAL) AS amount, i.due_date, i.status, i.created_at
        FROM invoices i
        JOIN contracts c ON i.contract_id = c.id
        JOIN rooms r ON c.room_id = r.id
        $where_clause
